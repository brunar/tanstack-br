	...
	return (
		<article className="flex items-center justify-between rounded-3xl border p-5">
			<div className="flex items-start gap-4 justify-between w-full">
				...
				<button
					className="inline-flex items-center justify-center rounded-full p-2 border border-gray-300 shadow-md bg-gray-100"
					onClick={() => setLiked((current) => !current)}
					type="button"
					aria-pressed={liked}
				>
					<Heart
						className={liked ? 'fill-current text-lime-500' : 'none'}
						size={18}
					/>
				</button>
			</div>
